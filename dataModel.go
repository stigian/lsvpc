...
type Instance struct {
	...
	RawEc2     types.Instance
	InstanceData
}

type NetworkInterfaceData struct {
	RawNetworkInterface types.NetworkInterface `json:"-"`
	ID                  string                 `json:"id"`
	PrivateIP           string                 `json:"privateIp"`
	MAC                 string                 `json:"mAC"`
	DNS                 string                 `json:"dNS"`
...
type SecurityGroup struct {
	RawSecurityGroup    types.SecurityGroup  `json:"-"`
	Description         string               `json:"description"`
...
type Volume struct {
	...
	Name       string       `json:"name"`
	RawVolume  types.Volume `json:"-"`
	KMSKeyID   string       `json:"kmsKeyId"`
	Size       int32        `json:"size"`
	Encrypted  bool         `json:"encrypted"`
}
type NatGatewayData struct {
	RawNatGateway types.NatGateway `json:"-"`
	ID            string           `json:"id"`
	PrivateIP     string           `json:"privateIP"`
	PublicIP      string           `json:"publicIP"`
	State         string           `json:"state"`
...
type RouteTable struct {
	RawRoute types.RouteTable `json:"-"`
	ID       string           `json:"id"`
	Default  string           `json:"default"`
}

type TGWAttachment struct {
	RawAttachment    types.TransitGatewayVpcAttachment `json:"-"`
...
type VPCPeer struct {
	RawPeer   types.VpcPeeringConnection `json:"-"`
	ID        string                     `json:"id"`
...
type InterfaceEndpointData struct {
	RawEndpoint types.VpcEndpoint `json:"-"`
	ID          string            `json:"id"`
